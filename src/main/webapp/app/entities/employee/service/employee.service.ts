...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'joiningDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      joiningDate: employee.joiningDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      joiningDate: restEmployee.joiningDate ? dayjs(restEmployee.joiningDate) : undefined,
    };
  }
